...
- `codexSync` — optional; exceptions to the managed harness baseline (the field keeps its original name). `enabled: false` skips the repository. `exclude` lists paths the sync leaves alone: non-empty strings, no duplicates. A repository that owns generated entries inside a managed JSON hook adapter declares their stable markers under `preserveJsonArrayEntries`: keys are `.json` paths that are not also excluded, values are non-empty arrays of unique, non-empty marker strings. Which paths are managed at all is decided by the harness-sync tooling that owns that inventory, not by this manifest.
